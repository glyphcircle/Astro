...
const NumerologyAstrology: React.FC<NumerologyAstrologyProps> = ({ mode }) => {
  ...
  const handleSmartCityChange = (city: string, newCoords?: { lat: number; lng: number }) => {
  ...
  return (
    <div className={`max-w-4xl mx-auto relative pb-24 transition-colors duration-500 ${isLight ? 'text-amber-950' : 'text-amber-50'}`}>
      ...
      {paymentSuccess && !isLoading && (
        <div className={`mb-6 p-4 rounded-lg text-center transition-all duration-1000 ${
          isLight ? 'bg-green-100 text-green-800 border-2 border-green-300' : 'bg-green-900/30 text-green-300 border-2 border-green-700'
        } animate-fade-out`}>
        ...
        <Card className={`mb-10 p-10 border-2 shadow-2xl transition-all duration-500 ${isLight ? 'bg-white/80 border-amber-200 shadow-amber-200/40' : 'border-amber-500/20'}`}>
          ...
          <form onSubmit={handleGetReading} className="grid md:grid-cols-2 gap-8 lg:gap-10">
            <div className="md:col-span-2">
              ...
              <input name="name" value={formData.name} onChange={handleInputChange} className={`w-full p-4 border-2 rounded-2xl text-lg outline-none transition-all ${isLight ? 'bg-amber-50/50 border-amber-200 text-amber-950 focus:border-amber-600 focus:bg-white' : 'bg-black/40 border-amber-500/20 text-white focus:border-amber-400 focus:ring-1 focus:ring-amber-500/50'}`} placeholder="Seeker's Name" required />
            </div>
          ...
          {isLoading && !isPaid ? (
            ...
            <>
              {mode === 'astrology' && engineData ? (
                <EnhancedAstrologyReport data={{ ...engineData, userName: formData.name, birthDate: formData.dob }} />
              ) : mode === 'numerology' && engineData ? (
                <EnhancedNumerologyReport reading={reading} engineData={engineData} userName={formData.name} birthDate={formData.dob} />
              ) : null}
            </>
          ) : isPaid && isLoading ? (
            <ReportLoader />
